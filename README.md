...
- ✅ 修复重启后媒体任务重复下载与状态不更新问题
- ✅ 优化 Telethon 消息匹配与失败提示，减少无效等待
- ✅ 增强持久化清理机制，降低 `tasks.json` 膨胀风险
- ✅ 精简命令与任务页交互（仅保留 `/clear_cache`，移除不适用按钮/状态）
...
- ✅ **核心下载流程稳定化** - Telegram 消息处理、下载链路、错误恢复
- ✅ **Web 管理界面可用** - 登录、任务页、基础状态展示
- ✅ **容器化部署完善** - Docker 启动与目录映射规范化
- ✅ **版本标签发布** - 提供固定版本与最新版本镜像标签
